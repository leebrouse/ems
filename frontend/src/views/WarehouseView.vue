...
const saveItem = async () => {
  ...
  if (itemDialogMode.value === "create") {
    ...
    await request.put(`/api/v1/items/${itemForm.id}`, {
      name: itemForm.name,
      unit: itemForm.unit,
      description: itemForm.description || undefined,
    });
...
const saveWarehouse = async () => {
  ...
  if (warehouseDialogMode.value === "create") {
    await request.post("/api/v1/warehouses", {
      name: warehouseForm.name,
      location: warehouseForm.location || undefined,
    });
    ElMessage.success("仓库已创建");
  } else {
    await request.put(`/api/v1/warehouses/${warehouseForm.id}`, {
      name: warehouseForm.name,
      location: warehouseForm.location || undefined,
    });
...
const saveThreshold = async () => {
  ...
  await request.put("/api/v1/alerts/threshold", {
    itemId: thresholdForm.itemId,
...
<template>
  <div class="warehouse-container">
    ...
    <el-dialog
      ...
      :title="itemDialogMode === 'create' ? '新建物资' : '编辑物资'"
      width="520px"
    >
    ...
    <el-dialog
      ...
      :title="warehouseDialogMode === 'create' ? '新建仓库' : '编辑仓库'"
      width="520px"
    >
      <el-form label-width="90px">
        <el-form-item label="名称">
